...
/**
 * Old climb code as the current code doesn't exist
...
public class Climber implements Loggable{
    ...
    public void climb(){ 
	/*
	 * We don't seem to need to climb right now, but here is the old code in case it is needed
	...
	String xmlString="<climber>\n";
        xmlString=xmlString.concat("<beginMotor>\n"+
               "<target>"+(ControlScheme.doSetForClimb()?1:0)+"</target>\n"+
               "<current>"+angler.get()+"</current>\n"+
                "</beginMotor>\n");
	xmlString=xmlString.concat("<climbMotor>\n"+
                "<target>"+((ControlScheme.climbUp()?1:0)-(ControlScheme.climbDown()?1:0))+"</target>\n"+
                "<current>"+climber.get()+"</current>\n"+
                "</climbMotor>\n");
	xmlString=xmlString.concat("<solenoid>\n"+
                "<state>"+(up.get())+"</state>\n"+
                "</solenoid>\n");
